...
import { IVideoCreateFormData, VideoMapper } from '../../types';
import { yupResolver } from '@hookform/resolvers/yup';
import React, { useState } from 'react';
import { useTranslation } from 'react-i18next';
import { useVideoTagOptions } from '../../hooks';
...
export function UploadZipModal({ loading, onSubmit }: Props) {
  ...
  const schema = React.useMemo(() => {
    return yup.object({
      ...
      compressedFile: yup
        .mixed()
        .required(t('video:videoImportZipModal.compressedFileRequired')),
    });
  }, []);

  const { control, handleSubmit, formState } = useForm<IVideoCreateFormData>({
  ...
  return (
    <form
      onSubmit={handleSubmit((data) => {
        onSubmit(videoMapper.toCreateVideoRequest(data, tagOptions));
      })}
      className={clsx({
    ...
    >
      <h1 className="text-lg font-semibold">
        {t('video:videoImportZipModal.onlyOneZipAllowed')}
      </h1>
      <Controller
        name="compressedFile"
        control={control}
      ...
      {uploadedFile && (
        <div className="grid items-center grid-cols-2 gap-4 px-5 py-4 rounded-lg h-fit bg-neutral-background-layer-2">
          ...
          <div className="flex flex-col items-start w-1/2 gap-4">
            <InfoCard label={t('video:basicInfo.dimension')} content={'-'} />
            <InfoCard
      ...
      <Button
        type="submit"
        text={t('video:videoImportZipModal.uploadZip')}
