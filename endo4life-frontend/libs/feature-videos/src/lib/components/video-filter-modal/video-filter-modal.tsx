...
import { VideoFiltersForm } from './video-filter-form';
import {
  FilterBuilder,
  FilterQueryBuilder,
  filterUtils,
...
export function VideoFilterDialog({ filter, onChange, onClose }: Props) {
  const { t } = useTranslation('video');

  const handleSubmit = (data: IVideoFilterFormData) => {
  ...
  const defaultFormData = useMemo<IVideoFilterFormData>(() => {
    return {
