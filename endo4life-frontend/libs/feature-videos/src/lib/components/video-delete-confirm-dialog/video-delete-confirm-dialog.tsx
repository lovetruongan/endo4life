...
export function VideoDeleteConfirmDialog({ id, onClose }: Props) {
  ...
  const deleteVideo = useCallback(async () => {
    if (!id) {
      toast.error(t('video:errors.invalidVideoId'), {
        position: 'top-right',
        autoClose: 2000,
    ...
    mutation.mutate(
      { id },
      {
        onSuccess(data, variables, context) {
          toast.success(t('video:videoDeleteConfirmDialog.status.success'), {
            position: 'top-right',
            autoClose: 2000,
            hideProgressBar: true,
          });
        ...
        onError(error, variables, context) {
          toast.error(t('video:videoDeleteConfirmDialog.status.error'), {
            position: 'top-right',
            autoClose: 2000,
            hideProgressBar: true,
          });
      ...
      <section className="w-full max-w-xl px-6 py-6 bg-white rounded-lg shadow">
        <header className="flex items-center gap-4">
          <h2 className="flex-auto text-2xl font-semibold">
            {t('video:videoDeleteConfirmDialog.title')}
          </h2>
        </header>
        <div className="py-4 space-y-2">
          <p>{t('video:videoDeleteConfirmDialog.confirmationMessage')}</p>
          <p>{t('video:videoDeleteConfirmDialog.cannotRestore')}</p>
        </div>
