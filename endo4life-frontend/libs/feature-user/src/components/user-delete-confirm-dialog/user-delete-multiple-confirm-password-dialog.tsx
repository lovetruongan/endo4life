...
export function UserDeleteMultipleConfirmPassword({
  users,
  onClose,
  ...
  const deleteUsers = useCallback(async () => {
    ...
    mutation.mutate(
      { ids: users.map((user) => user.id), password: password },
      {
  ...
  }, [mutation, onClose, navigate, onCloseConfirmDeleteDialog, password, t]);
